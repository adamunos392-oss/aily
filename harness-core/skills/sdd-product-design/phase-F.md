...
第 13 节「契约追踪」在阶段 F 只生成 `阶段 C 补充 | Pending` 占位行，不预填实际值。该占位由阶段 C 的 C3.5「回填 Feature Spec 契约追踪」统一兑现（数据模型实体、`API-Fxxx-xx` 编号或「无（纯前端）」、状态置 `Confirmed`）；阶段 F、B1、B2 均不回填此节。
