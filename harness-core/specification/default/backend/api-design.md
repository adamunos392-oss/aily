...
- 接口文档 = FastAPI 自动生成的 Swagger（`/docs`）；路由函数 + Pydantic 请求/响应模型写在代码里，代码即文档。`docs/api-contracts.md` 是产品设计阶段产出的契约权威，接口实现必须与其一致
...
- curl 验证命令的请求体/预期返回必须与 `/docs` 及 `docs/api-contracts.md` 中展示的模型结构一致
  - 验收：任取一个接口，`/docs` 中可见完整请求/响应结构，前端能仅凭契约写出 Mock，后端实现与契约一致并通过 curl 对照

## 统一响应信封

- 所有接口（含错误）返回统一信封结构：`{"code": "<错误码或OK>", "message": "<人类可读信息>", "data": <业务数据或null>}`；信封由 `pycore.api.responses` 统一实现（`success_response` / `error_response` / `paginated_response`），禁止路由手拼错误 JSON
- HTTP 状态码与信封 code 并用：HTTP 表达传输层语义，信封 code 表达业务语义，两者必须同时正确
- 前端解析只认信封结构，禁止对裸 JSON / 裸数组做特判
  - 正例：成功返回 `{"code": 200, "message": "ok", "data": {"id": 1}}`；失败返回 `{"code": "VALIDATION_ERROR", "message": "参数验证失败", "data": null}`
...
- **资源词从 PRD / Feature Map 的名词实体推导**：通读 PRD 功能清单，圈出被创建/操作的业务对象（名词实体），一个名词实体一个资源词；资源词一经推导，`docs/api-contracts.md` 的接口清单、URL 前缀与路由文件名共用同一个词，任何一侧不得从功能措辞另抓同义词
  - 正例：PRD 功能「会话问答」→ 名词实体 = 会话 → 资源词 `conversations` → `/api/conversations` + `api/routes/conversations.py`
  - 反例：同一 PRD 功能，api-contracts 写 `chat`、路由文件叫 `conversations`——两边各自从措辞抓词、无共同推导起点，属本条款违规（Tester 单列不一致项）
...
- 判定：grep `backend/src/api/` 直属层无含 `APIRouter` 的文件；`routes/` 下文件名与 api-contracts 接口清单的资源一一对应；每个路由文件只承载一个资源；接口清单与路由文件两边的资源词并集逐个回指 PRD 名词实体，回指不到的即自造词，违规
- 消费方：产品设计阶段（tech-spec §3 接口清单与 api-contracts 接口清单的资源词均按此推导，不从功能措辞另抓词）+ Developer（路由文件名与 URL 同源推导）+ Tester（审契约与路由路径/资源词一致性，不一致本身即 FAIL 项）
...
- 外部服务（LLM 如百炼、第三方 HTTP 服务）的 URL、模型名、请求/响应字段方案在方案阶段写死进产品设计产物（`docs/api-contracts.md` / `docs/Plan.md`），并落为 config 常量（config 中的配置项 + Pydantic 模型）
